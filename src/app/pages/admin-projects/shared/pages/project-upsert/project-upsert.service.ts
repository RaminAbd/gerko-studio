...
export class ProjectUpsertService {
  ...
  private statusesService: ProjectStatusesApiService = inject(ProjectStatusesApiService);
  ...
  getStatuses(){
    this.statusesService.GetAllByLang(this.statusesService.serviceUrl, this.translate.currentLang).subscribe(resp=>{
      this.component.statuses = resp.data;
      this.getTypes()
    })
  }
  private getTypes() {
    this.typesService.GetAllByLang(this.typesService.serviceUrl, this.translate.currentLang).subscribe(resp=>{
